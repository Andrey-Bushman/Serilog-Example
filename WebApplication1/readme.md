...
Проект создан на основе шаблона `ASP.NET Core Web API` на платформе `.Net 7`.
...
Примеры использования логгера (_с поясняющими комментариями_) смотрите в файле `WeatherForecastController.cs`.
  ...
  1. Свойства структурированного лога не записываются автоматически в текстовый файл. Для логирования в файлы в настройках `appsettings.json`
     вы можете указать нужный вам шаблон формирования текстовых сообщений. Делается это в настройке `Serilog > WriteTo > [outputTemplate]`. При этом вам
     доступны пользовательские свойства, программно определённые вами в коде в рамках `scope`. Однако, вместо использования `Serilog > WriteTo > [outputTemplate]`
     вы можете указать, что сообщения следует записывать в файл в виде компактных JSON-объектов. В этом случае в сообщение будут записаны все свойства,
     которые были вами назначены сообщению в рамках `Scope`. Настройки `Serilog > WriteTo > File > [formatter]` и `Serilog > WriteTo > [outputTemplate]`
     являются взаимоисключающими, т.е. присутствовать должна _только какая-то одна из них_. Пример настройки логгера для вывода сообщений в формате JSON
     вы можете найти в проекте **WorkerService1**.
...
Не забудьте предварительно установить на вашем компьютере приложение [Seq](https://datalust.co/seq). Первый запуск приложения **WebApplication1** следует
выполнять с правами администратора. Это необходимо для того, чтобы в **События Windows** смог быть добавлен новый _Источник событий_ = **WebApplication1**.
